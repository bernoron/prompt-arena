...
function genBetrieb(): string {
  // Lese .env.example und extrahiere alle Variablen mit Kontext
  const envExample = read(path.join(ROOT, '.env.example'));
  const envVars: { key: string; value: string; comment: string }[] = [];
  let pendingComment = '';
  for (const line of envExample.split('\n')) {
    const trimmed = line.trim();
    if (trimmed.startsWith('#')) {
      pendingComment = (pendingComment ? pendingComment + ' ' : '') + trimmed.replace(/^#+\s*/, '');
      continue;
    }
    const match = trimmed.match(/^([A-Z_]+)="?([^"]*)"?$/);
    if (match) envVars.push({ key: match[1], value: match[2], comment: pendingComment.trim() });
    if (!trimmed) pendingComment = '';
  }
  const envTable = envVars
    .map((v) => `| \`${v.key}\` | \`${v.value}\` | ${v.comment.split('.')[0] || '–'} |`)
    .join('\n');

  // Vorhandene Prisma-Migrationen
  const migrDir = path.join(ROOT, 'prisma', 'migrations');
  const migrations = readDir(migrDir)
    .filter((d) => d !== 'migration_lock.toml' && !d.endsWith('.md'))
    .sort()
    .map((d) => `- \`${d}\``)
    .join('\n') || '_(noch keine Migrationen vorhanden)_';

  return `# Betriebsdokumentation (DevOps)
