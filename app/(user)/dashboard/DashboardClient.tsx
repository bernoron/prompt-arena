...
interface DashboardClientProps {
  ...
  initialChallenges: WeeklyChallengeData[];
  initialTrending: PromptWithDetails[];
  initialLearn: LearningModuleWithProgress[];
}
...
//
// All four data sources (users, challenges, trending, learn) are fetched on the
// server (see page.tsx) and passed in as initial data, so there is NO
// fetch-on-mount waterfall. This component only layers on the client-only bits:
// the "since last visit" rank diff (needs localStorage) and the live countdown.

// @spec AC-04-004, AC-04-005
export default function DashboardClient({
  initialUsers, initialChallenges, initialTrending, initialLearn,
}: DashboardClientProps) {
  const currentUserId = useCurrentUser();

  const pickUser = useCallback(
    (users: UserWithStats[]): UserWithStats | null =>
      (currentUserId ? users.find((u) => u.id === currentUserId) : null) ?? users[0] ?? null,
    [currentUserId],
  );

  const [challenges,   setChallenges]   = useState<WeeklyChallengeData[]>(initialChallenges);
  const [currentUser,  setCurrentUser]  = useState<UserWithStats | null>(() => pickUser(initialUsers));
  const [allUsers,     setAllUsers]     = useState<UserWithStats[]>(initialUsers);
  const [allPrompts,   setAllPrompts]   = useState<PromptWithDetails[]>(initialTrending);
  const [rankDiff,     setRankDiff]     = useState<RankDiff | null>(null);
  const [learnModules, setLearnModules] = useState<LearningModuleWithProgress[]>(initialLearn);
  const [loading,      setLoading]      = useState(false);
  const [apiError,     setApiError]     = useState<string | null>(null);

  // Client-only enrichment: resolve the current user, compute the rank diff
  // against the last-visit snapshot (localStorage), and persist a fresh
  // snapshot. Runs on the server-provided data — no network round-trip.
  useEffect(() => {
    const user = pickUser(allUsers);
    setCurrentUser(user);
    if (!user) return;

    const currentRank = allUsers.findIndex((u) => u.id === user.id) + 1;
    ...
    if (snapshot) setRankDiff(computeDiff(allUsers, user.id, snapshot));
  ...
  // Retry path for the error banner — re-fetches everything from the API.
    ...
    <div className="space-y-6">
      ...
      {apiError && (
        <div role="alert" className="flex items-center gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">
          ...
          <button onClick={reload} className="font-semibold underline underline-offset-2 hover:text-rose-900">
            Neu laden
          </button>
        </div>
      )}
