...
interface LibraryClientProps {
  initialItems: PromptWithDetails[];
  initialNextCursor: number | null;
  initialHasNextPage: boolean;
  initialCategories: PromptCategoryInfo[];
}

// The first page of prompts and the category list are fetched on the server
// (see page.tsx) and passed in here, so the initial render needs no client
// fetch. This component only fetches when the user changes a filter/sort or
// scrolls for more — and never re-fetches the whole list after a mutation
// (optimistic updates + the single-prompt aggregate returned by the vote API).
function LibraryPageInner({
  initialItems, initialNextCursor, initialHasNextPage, initialCategories,
}: LibraryClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [prompts, setPrompts]         = useState<PromptWithDetails[]>(initialItems);
  ...
  const [categories] = useState<PromptCategoryInfo[]>(initialCategories);
  ...
  // The initial list matches the server-rendered default filters, so the first
  // run of the filter effect must NOT re-fetch (that would duplicate the work
  // already done on the server). Subsequent filter/sort/search changes do.
  ...
  // ── Filter/sort-change load (resets list) ────────────────────────────────────
  ...
  const handleVote = async (promptId: number, value: number) => {
    ...
    // Optimistic: the user's own star + a bumped count for a brand-new vote.
    ...
    try {
      ...
      if (res.ok) {
        // Merge the authoritative aggregate for just this prompt instead of
...
// @spec AC-02-012
export default function LibraryClient(props: LibraryClientProps) {
  return (
    <Suspense>
