...
/**
 * Leaderboard is purely presentational (ranked list + prompt-of-the-week) with
 * no client interactions, so it renders entirely on the server: the data is
 * fetched directly via the service layer and streamed in the initial HTML —
 * no client-side fetch waterfall, and effectively zero page JavaScript.
 */
...
export default async function LeaderboardPage() {
  ...
  const [users, page] = await Promise.all([
    listUsers(),
    listPrompts({ sortBy: 'newest', take: 100, resolvedUserId: null }),
  ]);

  const prompts = page.items as unknown as PromptWithDetails[];
  const oneWeekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const thisWeek = prompts.filter((x) => new Date(x.createdAt).getTime() > oneWeekAgo);
  const topPrompt =
    [...(thisWeek.length ? thisWeek : prompts)].sort((a, b) => b.avgRating - a.avgRating)[0] ?? null;

  const top10 = users.slice(0, 10);
  const [top1, top2, top3, ...rest] = top10;
  const meInTop10 = currentUserId != null && top10.some((u) => u.id === currentUserId);
