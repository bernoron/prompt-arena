...
export default function AdminChallenges() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Create/Edit Form */}
      {showForm && (
        <form onSubmit={handleSave} className="bg-white rounded-2xl border border-emerald-200 shadow-xs p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
              ...
              className="w-full px-3 py-2 text-sm rounded-xl border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-emerald-400" />
            <textarea required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Beschreibung der Aufgabe…" rows={3}
              className="w-full px-3 py-2 text-sm rounded-xl border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-emerald-400 resize-none" />
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {challenges.map((c) => (
            <div key={c.id} className={`bg-white rounded-2xl border shadow-xs p-5 ${c.isActive ? 'border-emerald-200' : 'border-slate-200'}`}>
