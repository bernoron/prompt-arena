...
export type LearningLessonSeed = {
  slug: string;
  order: number;
  points: number;
  title: string;
...
export type LearningModuleSeed = {
  ...
  order: number;
  description: string;
  lessons: LearningLessonSeed[];
};

  const b = (blocks: ContentBlock[]) => JSON.stringify(blocks);
      ...
      lessons: [
        ...
        },
        {
          ...
          content: b([
            ...
            { type: 'warning', content: 'Grenzen von LLMs: Das reine Sprachmodell kennt nur Informationen bis zu seinem Trainings-Cutoff, keine Faktenprüfung in Echtzeit (kann plausibel klingende Fehler produzieren – "Halluzinieren"), kein Zugriff auf interne Systeme, keine verlässlichen Berechnungen bei komplexen Zahlen ohne Werkzeuge.' },
            ...
            { type: 'tip', content: 'Viele KI-Tools sind heute keine reinen Sprachmodelle mehr, sondern Assistenten mit Werkzeugen: Websuche für aktuelle Informationen, Code-Ausführung für exakte Berechnungen, Datei- und Bildanalyse. Manche können sogar mehrstufig selbstständig arbeiten ("agentisch") – z.B. mehrere Suchen hintereinander ausführen oder Code schreiben und testen. Das schliesst einige der klassischen Grenzen, ändert aber nichts daran, dass du Ergebnisse immer prüfen solltest.' },
            { type: 'text', content: 'Faustregel: Nutze KI als smarten Assistenten, der dir beim Denken und Formulieren hilft – nicht als allwissendes Faktenlexikon. Überprüfe Zahlen, Namen und aktuelle Fakten immer selbst, auch wenn das Modell Werkzeuge wie Websuche genutzt hat.' },
          ]),
        },
        {
    ...
    lessons: [
      {
        ...
        content: b([
          { type: 'text', content: 'Moderne KI-Modelle können nicht nur Text, sondern auch Bilder "lesen". Diese Fähigkeit nennt man "Vision" oder "multimodale KI". Du kannst Fotos, Screenshots, Diagramme, Grafiken oder gescannte Dokumente direkt an das Modell übergeben — und es analysiert, beschreibt oder beantwortet Fragen dazu.' },
          ...
          { type: 'tip', content: 'Vision ist mittlerweile bei den grossen KI-Anbietern (Claude, ChatGPT, Gemini) Standard, aber die Qualität und die unterstützten Formate (PDF, mehrere Bilder gleichzeitig, Video) unterscheiden sich je nach Modellversion und Abo-Stufe. Kostenlose Versionen haben oft Limits bei Anzahl oder Grösse der Bilder. Prüfe immer, ob dein konkretes Tool die Funktion in der benötigten Form hat, bevor du ein wichtiges Projekt darauf aufbaust.' },
    ...
    lessons: [
      {
        ...
        content: b([
          ...
          { type: 'tip', content: 'Neben dem Chat-Prompting gibt es mittlerweile agentische Coding-Tools (z.B. Claude Code, GitHub Copilot im Agent-Modus), die selbstständig Dateien lesen, Code schreiben, Tests ausführen und bei Fehlern nachbessern — über mehrere Schritte hinweg, ohne dass du jeden Zwischenschritt einzeln anstossen musst. Die Prompting-Prinzipien aus dieser Lektion (klarer Kontext, konkrete Requirements, Einschränkungen benennen) gelten dort genauso, nur dass du sie oft einmal zu Beginn der Aufgabe statt bei jedem einzelnen Snippet formulierst.' },
        ]),
      },
      {
  ...
  modules.push({
    ...
    description: 'Claude, GPT, Gemini, offene Modelle — jedes hat Stärken. Lerne, wie du anhand von Aufgabe, Denkbedarf und Datenschutz das passende Modell wählst.',
    lessons: [
      {
        slug: 'claude-staerken', order: 1, points: 15,
        title: 'Claude: Stärken & wann einsetzen',
        content: b([
          { type: 'text', content: 'Claude (von Anthropic) ist für sein sehr grosses Kontextfenster, seine Schreibqualität und seine Fähigkeit zur Nuancierung bekannt. Während andere Modelle in einzelnen Nischen mithalten können, glänzt Claude bei komplexen Reasoning-Aufgaben, langen Dokumenten und – über Claude Code – bei eigenständigem, mehrstufigem Arbeiten an Programmieraufgaben.' },
          { type: 'example', label: 'Claude vs. andere Modelle für verschiedene Aufgaben', bad: 'Claude für alles nutzen — auch wenn ein anderes Tool besser oder günstiger wäre.', good: 'Claude ist stark bei: sehr lange Dokumente in einem Rutsch analysieren, nuancierte, strukturierte Texte schreiben, komplexe Reasoning-Aufgaben, sichere und verantwortungsvolle Antworten, agentisches Arbeiten (mehrere Schritte selbstständig planen und ausführen). Andere Tools nutzen bei: Echtzeit-Websuche mit Quellenangabe (z.B. Perplexity), Bild-Generierung (z.B. Midjourney).', explanation: 'Kein Modell ist in allem am besten. Der Schlüssel ist zu wissen, wann welches Modell die richtige Wahl ist.' },
          ...
          { type: 'tip', content: 'Anthropic bietet mehrere Claude-Grössen an: ein schnelles, günstiges Modell für Routineaufgaben (aktuell Haiku), ein ausgewogenes Modell für den Alltag (aktuell Sonnet) und ein besonders leistungsfähiges Modell für die anspruchsvollsten Aufgaben (aktuell Opus). Bei schwierigen Denkaufgaben kann Claude zudem im "erweiterten Denkmodus" länger nachdenken, bevor es antwortet – nützlich bei Mathematik, Logik oder komplexer Planung. Passe die Wahl an die Aufgabe an: für einfache Umformulierungen reicht das kleinste Modell.' },
        ]),
      },
      {
        slug: 'gpt-vergleich', order: 2, points: 15,
        title: 'GPT (OpenAI) & Unterschiede zu Claude',
        content: b([
          { type: 'text', content: 'GPT (OpenAI) war mit ChatGPT der Auslöser für den breiten KI-Boom. OpenAI bringt sein Modell regelmässig in neuen Versionen heraus und bietet darüber hinaus ein breites ChatGPT-Ökosystem: Custom GPTs für wiederkehrende Workflows, Code-Interpreter für Datenanalyse direkt im Chat, Connectors/Actions zu Drittsystemen und einen Agent-Modus, der mehrstufige Aufgaben selbstständig ausführt.' },
          { type: 'example', label: 'GPT vs. Claude: Wann welches wählen?', bad: 'Ich nutze nur ChatGPT weil es bekannter ist, auch wenn Claude für meine Aufgabe besser geeignet wäre.', good: 'ChatGPT / GPT ist stärker bei: Integration mit vielen Third-Party-Tools (Zapier, Make.com etc.), Custom GPTs für wiederkehrende Workflows, Code-Interpreter (Datenanalyse direkt im Chat), breitem Plugin-/Connector-Ökosystem.\n\nClaude ist stärker bei: langen Kontexten, Textnuancierung, sicherem Verhalten, Dokumentenarbeit, agentischer Programmierarbeit über Claude Code.', explanation: 'Für Automation-Workflows: oft GPT. Für tiefe Dokumentenarbeit und Coding-Agenten: oft Claude. Teste beide für deine spezifische Aufgabe — Benchmarks sind oft irreführend für reale Use-Cases.' },
          { type: 'tip', content: 'Alle grossen Anbieter aktualisieren ihre Modelle laufend: Ein Modellname von vor einem Jahr bezeichnet oft ein anderes Modell als heute. Wenn du auf Konsistenz angewiesen bist (z.B. für Produktion), nutze immer eine konkrete, versionsgebundene Modellbezeichnung statt nur den allgemeinen Namen — und prüfe bei Modellwechseln, ob deine Prompts noch wie erwartet funktionieren.' },
          { type: 'pattern', name: 'Modell-Benchmark erstellen', template: 'Ich will [Modell A] und [Modell B] für [meinen Use-Case] vergleichen.\n\nMeine Test-Prompts:\n1. [Typischer Prompt 1]\n2. [Typischer Prompt 2]\n3. [Edge-Case Prompt]\n\nBitte beantworte jeden Prompt und ich vergleiche dann Qualität, Geschwindigkeit und Kosten.', example: 'Ich vergleiche Claude und die aktuelle GPT-Version für unsere Kundenkommunikations-Aufgaben. Zeige mir deine Antwort auf: 1. Beschwerde-Mail formulieren. 2. Technische Anfrage für Laien erklären. 3. Einen aggressiven Kunden deeskalieren.', useCase: 'Bevor du dich für ein Modell für einen Business-Prozess entscheidest.' },
        ]),
      },
      {
        ...
        content: b([
          ...
          { type: 'pattern', name: 'Spezialisiertes Modell wählen', template: 'Meine Aufgabe: [Aufgabe beschreiben]\n\nNutze spezialisierte Tools wenn vorhanden:\n- Code (autonom, im Terminal/IDE): Claude Code, GitHub Copilot, Cursor\n- Bilder: Midjourney, Stable Diffusion, das Bildmodell deines Chat-Tools\n- Präsentationen: Beautiful.ai, Gamma.app\n- Recherche mit Quellenangabe: Perplexity, You.com\n- Spreadsheets: Gemini in Google Sheets, Copilot in Excel', example: 'Aufgabe: Eine Marktanalyse erstellen. Vorgehen: Perplexity für aktuelle Marktdaten-Recherche (Websuche mit Quellen), dann Claude für strukturierten Bericht, dann Gamma.app für Präsentation. 3 spezialisierte Tools, optimal eingesetzt.', useCase: 'Komplexe Aufgaben die verschiedene Stärken brauchen: Recherche + Schreiben + Visualisierung.' },
          { type: 'tip', content: 'Perplexity.ai ist unterschätzt: Es kombiniert Websuche mit KI-Antworten und gibt immer Quellen an. Für aktuelle Informationen (was passiert heute? neueste Studien?) ist es oft eine gute Ergänzung zu Chat-Modellen ohne eigene Websuche — auch wenn mittlerweile auch Claude, ChatGPT und Gemini eigene Websuche-Funktionen anbieten.' },
        ]),
      },
      {
        ...
        content: b([
          { type: 'text', content: 'Meta\'s Llama, Mistral, DeepSeek, Qwen und viele weitere Open-Source-Modelle können lokal auf deinem Computer oder auf eigenen Servern betrieben werden. Das bedeutet: maximaler Datenschutz (keine Daten verlassen dein Gerät), keine Nutzungsgebühren, volle Kontrolle — aber auch: höhere technische Hürde und oft niedrigere Qualität als die grössten Cloud-Modelle.' },
          ...
          { type: 'tip', content: 'Für Einsteiger: Ollama (ollama.ai) ermöglicht das einfache Ausführen von Llama, Mistral, Gemma und anderen Modellen lokal auf dem eigenen Laptop — mit einer Zeile Terminal. Qualität: gut für viele Aufgaben, die Lücke zu den grössten Cloud-Modellen schliesst sich zwar stetig, ist aber bei den anspruchsvollsten Aufgaben noch spürbar.' },
          { type: 'pattern', name: 'Modell-Entscheidungsbaum', template: 'Für meine Aufgabe [Beschreibung] frage ich mich:\n\n1. Enthält die Aufgabe sensible Daten? → JA: lokales/Enterprise-Modell nötig\n2. Brauche ich aktuelle Web-Infos mit Quellenangabe? → JA: Tool mit Websuche (z.B. Perplexity)\n3. Ist es primär ein Code-Task, den ich weitgehend selbstständig erledigen lassen will? → JA: Coding-Agent wie Claude Code / GitHub Copilot\n4. Ist es ein langer Dokument-Task? → JA: Claude (sehr grosses Kontextfenster)\n5. Ist es in Google Workspace? → JA: Gemini for Workspace\n6. Sonst: Standard-Chat-Modell deiner Wahl (Claude, GPT, Gemini) je nach Präferenz', example: 'Aufgabe: Kundendaten aus PDFs extrahieren. Schritt 1: Sensible Daten? JA → Enterprise-Lösung oder Anonymisierung. Schritt 2: Web? NEIN. Schritt 3: Code? NEIN. Schritt 4: Lange Dokumente? JA → Claude mit PDF-Upload.', useCase: 'Immer wenn du ein neues Projekt oder einen neuen Use-Case angehen willst und nicht sicher bist welches Tool.' },
        ]),
      },
    ],
  });

export const learningModules: LearningModuleSeed[] = modules;
